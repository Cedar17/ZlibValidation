...
/**
 * @brief Generates a SPICE library file from a given library file, applying a specified chain
 * length and cell names.
 *
 * This function takes a library file path, a log file name, a chain length, a vector of cell names,
 * a Verilog library file path, and a SPICE library file path as input. It initializes a LibFile
 * object, validates the chain length, and then calls the spice method of the LibFile object to
 * generate the SPICE library file. It logs the start and end of the SPICE generation process, as
 * well as any errors that occur.
 *
 * @param library_path The path to the input library file.
 * @param log_file_name The name of the log file. If empty, a default log file name is generated
...
/**
 * @brief Performs a functional equivalence check between two files (Liberty or Verilog) for a given
 * set of cells.
 *
 * This function compares the logic functions of specified cells in two files, which can be either
 * in Liberty (.lib) or Verilog (.v) format. It extracts the logic functions for each cell from both
 * files, compares them, and generates a report summarizing the comparison results.
 *
 ...
 * @param report_file_name A string to store the name of the report file. If empty, a default name
 * is generated. If the provided name does not end with ".txt" or ".md", ".md" is appended.
 *
 * @details
 * The function first checks the file extensions to determine the file format. It then extracts the
 * logic functions for each specified cell from both files.  The logic functions are then compared,
 * and a report is generated, which includes the comparison results for each cell. The report is
 * written to the specified report file.
 *
 * @note
 * - If no cell names are provided, the function logs an error and returns.
 * - If the reference or comparison file format is not supported (i.e., not .lib or .v), the
 * function logs an error and returns.
 ...
 * - The function utilizes the LogicComparator class to perform the logic comparison and generate
 * the report.
 * - Memory allocated for LibFile objects is managed using raw pointers and must be manually
