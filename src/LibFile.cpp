...
namespace { // Keep helper functions private to this compilation unit
...
bool modifySpiceNetlist(const std::string &spiceFile, const std::string &targetGlobalLine,
                        std::shared_ptr<spdlog::logger> logger) // Pass logger
{
  logger->info("Modifying SPICE netlist: {}", spiceFile);
  std::string tempFile = spiceFile + ".tmp"; // Temporary output file

  std::ifstream inFile(spiceFile);
  std::ofstream outFile(tempFile);

  if (!inFile) {
    logger->error("Could not open input SPICE file for modification: {}", spiceFile);
    return false;
  }
  if (!outFile) {
    logger->error("Could not open temporary output SPICE file: {}", tempFile);
  ...
  while (std::getline(inFile, line)) {
    ...
    if (line[0] == '*') {
      ...
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = splitString(previousInstanceLine);
        // Basic check: needs instance name, pins..., module name
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          // *** Assumption: The relevant output pin for the *next* stage or final connection
          // *** is the second to last token in the v2lvs output line.
          std::string outputNet = tokens[tokens.size() - 2];
          // Determine if it's a final stage based on typical output port names
          bool isFinal = (outputNet == "CO" || outputNet == "S" || outputNet == "ZN");

          // Modify output pin name (add :1) and add VDD VSS
          tokens[tokens.size() - 2] = outputNet + ":1";
          tokens.push_back("VDD");
          tokens.push_back("VSS");

          // Reconstruct and write modified instance line
          for (size_t i = 0; i < tokens.size(); ++i) {
            outFile << tokens[i] << (i == tokens.size() - 1 ? "" : " ");
          }
          outFile << std::endl;

          // Generate and write RC lines for the processed instance
          generateRCLines(outFile, outputNet, instanceIndex - 1,
                          isFinal); // instanceIndex was already incremented
        } else {
          logger->warn("Previous line stored but not recognized as instance: {}",
                       previousInstanceLine);
          outFile << previousInstanceLine << std::endl; // Write unmodified if not recognized
        }
    ...
    // Handle .INCLUDE: Write it and set flag to insert global line next
    ...
    if (includeFound && !globalInserted) {
      outFile << targetGlobalLine << std::endl;
      globalInserted = true;
      // Fall through to process the current line
    }

    // Handle .SUBCKT: Add VDD VSS to ports, reset instance counter
    if (line.find(".SUBCKT") == 0) {
      // Process any pending instance from the *previous* subcircuit's end
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          ...
          outFile << std::endl;
          generateRCLines(outFile, outputNet, instanceIndex - 1, isFinal);
        } else {
          logger->warn("Previous line stored but not recognized as instance: {}",
                       previousInstanceLine);
    ...
    // Handle .ENDS: Process the last instance before writing .ENDS
    else if (line.find(".ENDS") == 0) {
      inSubckt = false;
      ...
      outFile << line << std::endl; // Write the .ENDS line
    }
    // Skip original .GLOBAL lines generated by v2lvs -s0/-s1
    else if (line.find(".GLOBAL") == 0) {
      continue;
    }
    // Handle Instance lines (starting with X)
    else if (inSubckt && (line[0] == 'X' || line[0] == 'x')) {
      // Process the *previous* instance line (if any) before storing the current one
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          ...
          tokens[tokens.size() - 2] = outputNet + ":1"; // Modify output pin name
          tokens.push_back("VDD");
          tokens.push_back("VSS");
          ...
          outFile << std::endl;
          generateRCLines(outFile, outputNet, instanceIndex - 1, isFinal); // Use previous index
        } else {
          logger->warn("Previous line stored but not recognized as instance: {}",
                       previousInstanceLine);
          outFile << previousInstanceLine << std::endl;
      ...
      // Store current instance line for next iteration, increment index
      previousInstanceLine = line;
      instanceIndex++;
    }
    // Handle other lines (should not happen often in this structure)
    else {
      // Process previous instance if needed before writing this line
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
  ...
  // Process the very last instance line if the file didn't end with .ENDS or other processing
  // trigger
  if (!previousInstanceLine.empty()) {
    std::vector<std::string> tokens = splitString(previousInstanceLine);
    if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
      ...
      logger->warn("Last stored line not recognized as instance: {}", previousInstanceLine);
      outFile << previousInstanceLine << std::endl; // Write unmodified if not recognized
    }
  }
  ...
  if (std::remove(spiceFile.c_str()) != 0) {
    logger->warn("Could not remove original SPICE file: {}. Keeping temporary file: {}", spiceFile,
                 tempFile);
    // Keep the temp file if original cannot be removed
  } else {
    if (std::rename(tempFile.c_str(), spiceFile.c_str()) != 0) {
      logger->error("Could not rename temporary SPICE file {} to {}. Check permissions.", tempFile,
                    spiceFile);
      return false; // Failed to finalize the modification
    }
  }

  logger->info("SPICE netlist modification successful for: {}", spiceFile);
  return true;
}
...
void LibFile::spice(const int chain_length, const std::vector<std::string> &cell_names,
  ...
  // Generate the temporary Verilog file first
  this->verilog(chain_length, cell_names);

  // Define input and output filenames
  std::string temp_verilog_file = basename_ + "_temp.v";
  std::string output_spice_file = basename_ + ".spi";

  // Check if temporary Verilog file exists (generated by this->verilog)
  ...
  if (!pipe) {
    ...
    // Clean up temporary verilog file if popen fails
    std::remove(temp_verilog_file.c_str());
    return;
  }
  ...
  if (v2lvs_path_result.empty()) {
    ...
    // Clean up temporary verilog file if v2lvs is not found
    std::remove(temp_verilog_file.c_str());
    return;
  } else {
  ...
  // Construct and Run V2LVS command
  ...
  v2lvs_cmd_full += " -o " + output_spice_file; // Output SPICE
  ...
  // Clean up the temporary Verilog file regardless of v2lvs success/failure
  if (std::remove(temp_verilog_file.c_str()) != 0) {
    logger_->warn("Could not remove temporary Verilog file: {}", temp_verilog_file);
  }

  if (ret != 0) {
    // WEXITSTATUS might be needed for more specific error codes on POSIX
    logger_->error("V2LVS command failed with exit code: {}. Command: {}", ret, v2lvs_cmd_full);
    // Attempt to remove potentially incomplete output SPICE file
    std::remove(output_spice_file.c_str());
    return;
  }
  logger_->info("V2LVS completed successfully.");

  // --- Post-process the generated SPICE file ---
  std::string target_global =
      ".global VSS GND COREGND1 VDD COREVDD1"; // Define the desired global line

  // Call the modification function
  if (!modifySpiceNetlist(output_spice_file, target_global, logger_)) {
    logger_->error("Failed to modify the generated SPICE netlist: {}", output_spice_file);
    // Output file might be in an intermediate state or removed/renamed incorrectly
    return; // Stop processing if modification fails
  }

  // Final success message
  logger_->info("SPICE generation and modification complete in '{}'", output_spice_file);
}
