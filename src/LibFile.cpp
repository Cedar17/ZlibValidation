...
/**
 * @brief Splits a string into a vector of tokens, using whitespace as the delimiter.
 *
 * This function takes a string as input and splits it into a vector of strings,
 * where each element in the vector represents a token from the original string.
 * Whitespace characters (spaces, tabs, newlines, etc.) are used as delimiters
 * to separate the tokens.
 *
 * @param s The string to be split.
 * @return A vector of strings, where each string is a token from the input string.
 */
std::vector<std::string> LibFile::splitString(const std::string &s) {
...
void LibFile::generateRCLines(std::ofstream &outFile, const std::string &netName, int instanceIndex,
                              bool isFinalStage) {
  // --- Default RC Values (as observed in the target SPICE) ---
  const double R1_INTERMEDIATE = 0.01;
  const double C1_INTERMEDIATE = 5.3e-16;
  const double R2_INTERMEDIATE = 0.01;
  const double R1_FINAL = 1e-2;
  const double C1_FINAL = 1e-18;
  const std::string CAP_GROUND = "COREGND1"; // Assumed ground for capacitors

  std::string r1Name = "R1_" + std::to_string(instanceIndex);
  std::string c1Name = "C1_" + std::to_string(instanceIndex);
  std::string r2Name = "R2_" + std::to_string(instanceIndex);
  std::string node1 = netName + ":1"; // Intermediate node 1
  std::string node2 = netName + ":2"; // Intermediate node 2

  // Set precision for scientific notation output
  outFile << std::scientific << std::setprecision(1);

  if (!isFinalStage) {
    // Intermediate RCR structure (R1-C1-R2)
    outFile << r1Name << " " << node1 << " " << node2 << " " << R1_INTERMEDIATE << std::endl;
    outFile << c1Name << " " << node2 << " " << CAP_GROUND << " " << C1_INTERMEDIATE << std::endl;
    outFile << r2Name << " " << node2 << " " << netName << " " << R2_INTERMEDIATE << std::endl;
  } else {
    // Final RC structure (R1-C1 connected to the instance output node :1)
    // Note: The target SPICE connects R1 between node1 and the final port (netName).
    outFile << r1Name << " " << node1 << " " << netName << " " << R1_FINAL << std::endl;
    outFile << c1Name << " " << node1 << " " << CAP_GROUND << " " << C1_FINAL << std::endl;
  }
  // Reset precision/format to default
  outFile << std::defaultfloat << std::setprecision(6);
}

/**
 * @brief Modifies a SPICE netlist generated by v2lvs, adding metadata,
 * inserting a global line, and adjusting instance lines within subcircuits to
 * include VDD/VSS connections and generate corresponding R/C lines.
 *
 * This function reads a SPICE netlist from `v2lvsSpiceFile`, modifies it
 * according to the following rules, and writes the result to
 * `finalSpiceFile`:
 *
 * 1.  **Metadata Comments:** Adds a header with application name, version,
 *     author, and timestamp.
 * 2.  **Global Line Insertion:** Inserts a specified global line
 *     (`targetGlobalLine`) after the first `.INCLUDE` directive.
 * 3.  **Subcircuit Modifications:**
 *     *   Adds `VDD VSS` to the port list of `.SUBCKT` definitions.
 *     *   For each instance line (starting with 'X' or 'x') within a
 *         subcircuit:
 *         *   Modifies the output pin name by appending ":1".
 *         *   Inserts `VDD VSS` before the module name in the instance line.
 *         *   Calls `generateRCLines` to generate and insert R/C lines for the
 *             instance.
 * 4.  **Comment Handling:** Preserves original comments, but processes any
 *     pending instance line before writing the comment.
 * 5.  **Empty/v2lvs Header Line Skipping:** Skips empty lines and lines
 *     starting with v2lvs header comments.
 * 6.  **.GLOBAL Line Skipping:** Skips original .GLOBAL lines.
 *
 * The function handles case-insensitive `.SUBCKT` and `.ENDS` directives. It
 * also manages a `previousInstanceLine` buffer to handle instance lines that
 * might need modification before the next line is processed.
 *
 * @param v2lvsSpiceFile The path to the input SPICE netlist file generated by
 * v2lvs.
 * @param finalSpiceFile The path to the output SPICE netlist file after
 * modification. This file will be overwritten if it exists.
 * @param targetGlobalLine The string containing the global line to be inserted
 * after the `.INCLUDE` directive.
 * @return `true` if the modification was successful, `false` otherwise (e.g.,
 * if file opening fails).
 *
 * @note The function assumes that the second to last token in an instance line
 * is the output net. It also assumes a specific format for instance lines
 * where VDD/VSS insertion and R/C line generation are applicable.  The
 * `generateRCLines` method is expected to handle the actual generation of R/C
 * components.
 *
 * @see generateRCLines
 */
bool LibFile::modifySpiceNetlist(const std::string &v2lvsSpiceFile, // Input is the v2lvs output
                                 const std::string &finalSpiceFile, // Output is the final file
                                 const std::string &targetGlobalLine) {
  logger_->info("Post-processing SPICE netlist: {} -> {}", v2lvsSpiceFile, finalSpiceFile);

  std::ifstream inFile(v2lvsSpiceFile);
  // Write directly to the final output file, overwriting if it exists
  std::ofstream outFile(finalSpiceFile, std::ios::trunc);

  if (!inFile) {
    logger_->error("Could not open input v2lvs SPICE file for modification: {}", v2lvsSpiceFile);
    return false;
  }
  if (!outFile) {
    logger_->error("Could not open final output SPICE file for writing: {}", finalSpiceFile);
    inFile.close(); // Close input file if output fails
    return false;
  }

  // --- Add Metadata Comments ---
  outFile << "** Generated by " << APP_NAME << " v" << APP_VERSION << " from " << APP_AUTHOR;
  auto now = std::chrono::system_clock::now();
  std::time_t now_time_t = std::chrono::system_clock::to_time_t(now);
  // Use localtime_s on Windows, localtime_r on POSIX, or std::localtime (less safe)
  // std::tm now_tm;
  // localtime_s(&now_tm, &now_time_t); // Example for Windows
  std::tm *now_tm_ptr = std::localtime(&now_time_t); // Standard C++, potentially less thread-safe
  if (now_tm_ptr) {
    outFile << ". On: " << std::put_time(now_tm_ptr, "%c %Z") << " **\n" << std::endl;
  } else {
    outFile << ". Timestamp unavailable **\n" << std::endl;
  }
  // --- End Metadata ---

  std::string line;
  bool includeFound = false;
  bool globalInserted = false;
  bool inSubckt = false;
  std::string previousInstanceLine = "";
  int instanceIndex = 0; // Counter for R/C naming within a subckt
  ...
  while (std::getline(inFile, line)) {
    ...
    // Skip empty lines and v2lvs header comments
    if (line.empty() || line.find("$ Spice netlist generated by v2lvs") == 0 ||
        line.find("$ v2") == 0 || // Catch version line too
        line.find("*.BUSDELIMITER") == 0) {
      continue; // Skip these lines entirely
    }

    // Write other comments directly after processing any pending instance
    if (line[0] == '*') {
      // Process previous instance before writing comment if needed
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = this->splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          std::string outputNet = tokens[tokens.size() - 2]; // Assumption
          bool isFinal = (outputNet == "CO" || outputNet == "S" || outputNet == "ZN");
          tokens[tokens.size() - 2] = outputNet + ":1"; // Modify output pin

          // Reconstruct line with correct VDD/VSS order
          for (size_t i = 0; i < tokens.size() - 1; ++i) { // Up to module name
            outFile << tokens[i] << " ";
          }
          outFile << "VDD VSS " << moduleName << std::endl; // Insert VDD VSS before module name

          this->generateRCLines(outFile, outputNet, instanceIndex - 1, isFinal);
        } else {
          logger_->warn("Previous line stored but not recognized as instance: {}",
                        previousInstanceLine);
          outFile << previousInstanceLine << std::endl;
        }
        previousInstanceLine = ""; // Clear after processing
      }
      outFile << line << std::endl; // Write the comment line
      continue;
    }
    ...
    if (line.find(".INCLUDE") == 0) {
      ...
      includeFound = true;
      continue; // Process next line to insert global
    }

    // Insert the target global line after .INCLUDE
    if (includeFound && !globalInserted) {
      outFile << targetGlobalLine << std::endl << std::endl; // Add extra newline for spacing
      globalInserted = true;
      // Fall through to process the current line
    }

    // Handle .SUBCKT
    if (line.find(".SUBCKT") == 0 || line.find(".subckt") == 0) { // Handle case-insensitivity
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = this->splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          std::string outputNet = tokens[tokens.size() - 2];
          bool isFinal = (outputNet == "CO" || outputNet == "S" || outputNet == "ZN");
          tokens[tokens.size() - 2] = outputNet + ":1";

          for (size_t i = 0; i < tokens.size() - 1; ++i) {
            outFile << tokens[i] << " ";
          }
          outFile << "VDD VSS " << moduleName << std::endl;
      ...
      inSubckt = true;
      instanceIndex = 0;                          // Reset R/C counter for new subcircuit
      outFile << line << " VDD VSS" << std::endl; // Add VDD VSS to ports
    }
    // Handle .ENDS
    else if (line.find(".ENDS") == 0 || line.find(".ends") == 0) { // Handle case-insensitivity
      inSubckt = false;
      // Process the last instance line *before* writing .ENDS
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = this->splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
      ...
      outFile << line << std::endl << std::endl; // Add extra newline after .ENDS
    }
    // Skip original .GLOBAL lines
    else if (line.find(".GLOBAL") == 0 || line.find(".global") == 0) {
      continue;
    }
    // Handle Instance lines
    else if (inSubckt && (line[0] == 'X' || line[0] == 'x')) {
      if (!previousInstanceLine.empty()) {
        std::vector<std::string> tokens = this->splitString(previousInstanceLine);
        if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
          std::string moduleName = tokens.back();
          ...
          this->generateRCLines(outFile, outputNet, instanceIndex - 1,
                                isFinal); // Use previous index
        } else {
          logger_->warn("Previous line stored but not recognized as instance: {}",
                        previousInstanceLine);
          outFile << previousInstanceLine << std::endl;
  ...
  // Process the very last instance line
  if (!previousInstanceLine.empty()) {
    std::vector<std::string> tokens = this->splitString(previousInstanceLine);
    if (tokens.size() > 2 && (tokens[0][0] == 'X' || tokens[0][0] == 'x')) {
      std::string moduleName = tokens.back();
      ...
      logger_->warn("Last stored line not recognized as instance: {}", previousInstanceLine);
      outFile << previousInstanceLine << std::endl;
    }
  }
  ...
  // No renaming needed, we wrote directly to the final file
  logger_->info("SPICE netlist post-processing successful for: {}", finalSpiceFile);
  return true;
}
...
void LibFile::spice(const int chain_length, const std::vector<std::string> &cell_names,
  ...
  // 1. Generate the temporary Verilog file first
  this->verilog(chain_length, cell_names);

  // Define input and output filenames
  std::string temp_verilog_file = basename_ + "_temp.v";
  std::string v2lvs_output_spice_file = basename_ + ".v2lvs.spi"; // v2lvs output
  std::string final_output_spice_file = basename_ + ".spi";       // Final processed output

  // Check if temporary Verilog file exists
  std::ifstream check_v_in(temp_verilog_file);
  if (!check_v_in.is_open()) {
    logger_->error("Temporary Verilog file {} not found or could not be opened. Verilog generation "
                   "might have failed.",
                   temp_verilog_file);
    return;
  }
  check_v_in.close(); // Close after checking

  // 2. Check if V2LVS tool is available
  logger_->debug("Checking for v2lvs tool...");
  std::string v2lvs_path_cmd = "which v2lvs";
  std::string v2lvs_path_result;
  std::array<char, 128> buffer;
  std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(v2lvs_path_cmd.c_str(), "r"), pclose);
  if (!pipe) {
    logger_->error("Failed to run command to find v2lvs: {}", v2lvs_path_cmd);
    // Do NOT delete temp_verilog_file here if popen fails
    return;
  }
  while (fgets(buffer.data(), buffer.size(), pipe.get()) != nullptr) {
    v2lvs_path_result += buffer.data();
  }

  // Trim potential newline from the result
  if (!v2lvs_path_result.empty() && v2lvs_path_result.back() == '\n') {
    v2lvs_path_result.pop_back();
  }

  if (v2lvs_path_result.empty()) {
    logger_->error(
        "V2LVS tool not found in PATH. Please install Calibre or ensure v2lvs is accessible.");
    // Do NOT delete temp_verilog_file
    return;
  } else {
    logger_->info("V2LVS tool found at: {}", v2lvs_path_result);
  }

  // 3. Construct and Run V2LVS command
  logger_->info("Running V2LVS to generate initial SPICE netlist...");
  std::string v2lvs_cmd_full = "v2lvs";               // Start with the command name
  v2lvs_cmd_full += " -v " + temp_verilog_file;       // Input Verilog
  v2lvs_cmd_full += " -o " + v2lvs_output_spice_file; // Output SPICE
  v2lvs_cmd_full += " -s0 VSS";                       // Default ground net, generates .GLOBAL VSS
  v2lvs_cmd_full += " -s1 VDD";                       // Default power net, generates .GLOBAL VDD
  v2lvs_cmd_full += " -i";                            // Use positional pins (traditional SPICE)
  v2lvs_cmd_full += " -l " + verilog_lib_file;        // Verilog library for pin order
  v2lvs_cmd_full += " -s " + spice_lib_file;          // SPICE library to include

  logger_->debug("Executing V2LVS command: {}", v2lvs_cmd_full);
  int ret = system(v2lvs_cmd_full.c_str());

  // Do NOT delete temp_verilog_file

  if (ret != 0) {
    logger_->error("V2LVS command failed with exit code: {}. Command: {}", ret, v2lvs_cmd_full);
    // Attempt to remove potentially incomplete v2lvs output SPICE file
    std::remove(v2lvs_output_spice_file.c_str());
    return;
  }
  logger_->info("V2LVS completed successfully, output at '{}'.", v2lvs_output_spice_file);

  // 4. Post-process the generated SPICE file
  std::string target_global = ".global VSS GND COREGND1 VDD COREVDD1";

  // Call the modification function, reading from .v2lvs.spi and writing to .spi
  if (!this->modifySpiceNetlist(v2lvs_output_spice_file, final_output_spice_file, target_global)) {
    logger_->error("Failed to post-process the generated SPICE netlist: {}",
                   v2lvs_output_spice_file);
    // Keep both files for debugging if modification fails
    return;
  }

  // 5. Final success message
  logger_->info("Intermediate v2lvs output kept in '{}'", v2lvs_output_spice_file);
  logger_->info("Temporary Verilog input kept in '{}'", temp_verilog_file);
  logger_->info("SPICE generation and post-processing complete. Final output in '{}'",
                final_output_spice_file);
}

/**
 * @brief Retrieves the logic functions for the output pins of a specified cell within the Liberty
 * file.
 *
 * This method searches for a cell with the given name in the parsed Liberty file (either by parsing
 * the file directly or reading from a JSON representation). It then iterates through the output
 * pins of the cell, extracting the logic function associated with each pin. The logic functions are
 * stored in a map, where the key is the pin name and the value is the logic function string.
 *
 * If the JSON file does not exist, the Liberty file is parsed, and a JSON file is created. If the
 * JSON file exists but cannot be opened or parsed, an error is logged, and an empty map is
 * returned. If the specified cell is not found or if no logic functions are found for the cell, a
 * warning is logged.
 *
 * @param cell_name The name of the cell for which to retrieve the logic functions.
 * @return A map containing the logic functions for the output pins of the specified cell. The keys
 * of the map are the pin names, and the values are the corresponding logic function strings.
 * Returns an empty map if no logic functions are found or if an error occurs during file
 * processing.
 */
